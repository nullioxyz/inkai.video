...
describe('credits mappers', () => {
  ...
  it('maps video generation and normalizes optional nested fields', () => {
    const result = mapCreditVideoGenerationToViewModel({
      ...
      prediction: { id: 2, status: 'succeeded', output_video_url: 'https://inkai.video/video/credit-token/result.mp4' },
      credits_debited: 1,
      credits_refunded: 0,
      credits_used: 1,
      is_failed: false,
    ...
    expect(result.outputVideoUrl).toBe('https://inkai.video/video/credit-token/result.mp4');
    expect(result.creditEvents[0].delta).toBe(-1);
    expect(result.creditEvents[0].referenceType).toBe('input_creation');
  });

  it('keeps legacy relative output_video_url unchanged', () => {
    const result = mapCreditVideoGenerationToViewModel({
      input_id: 11,
      title: 'video',
      status: 'done',
    ...
    expect(result.outputVideoUrl).toBe('/storage/output.mp4');
  });
